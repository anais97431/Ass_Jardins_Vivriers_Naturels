...
function img_load($id_product)
{
        ...
        foreach ($_FILES['photos']['name'] as $key => $val) {
            ...
            if (in_array($fileType, $allowTypes)) {
                ...
                if (move_uploaded_file($_FILES["photos"]["tmp_name"][$key], $targetFilePath)) {
                    // Image db insert sql
                    $insertValuesSQL .= "('" . $fileName . "', $id_product),";
                } else {
    ...
    WHERE id_product=$id_product";
        ...
        if (!empty($insertValuesSQL)) {
            ...
            $insert = $connection->query("INSERT INTO shop_picture (title_picture, id_product) VALUES $insertValuesSQL");
            if ($insert) {
